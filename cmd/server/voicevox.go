...
func NewVoiceVox() (*VocevoxCore, error) {
	ret := &VocevoxCore{}
	ret.dll = windows.NewLazyDLL(GetDllDict())
	ret.func_initialize = ret.dll.NewProc("voicevox_initialize")
	ret.func_tts = ret.dll.NewProc("voicevox_tts")
	ret.func_wav_free = ret.dll.NewProc("voicevox_wav_free")
	ret.func_finalize = ret.dll.NewProc("voicevox_finalize")

	fmt.Println("coreの初期化中")
	ret.init_options = voicevox_make_default_initialize_options()
	ret.init_options.open_jtalk_dict_dir = []byte(GetOpenJTalkDict())
...
func (v *VocevoxCore) Generate(speak_words string) error {
	fmt.Println("音声生成中")
	speaker_id := 0
